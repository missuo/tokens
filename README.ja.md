...
> **[`bunx tokens-cli submit`](#ソーシャルプラットフォームコマンド)を実行して、使用量データをリーダーボードに送信し、公開プロフィールを作成しましょう！**
...
| <img width="48px" src=".github/assets/client-cursor.jpg" alt="Cursor" /> | [Cursor IDE](https://cursor.com/) | `~/.config/tokens/cursor-cache/`経由でAPI同期 | ✅ 対応 |
...
> **パッケージ構造**: npmパッケージ名は`tokens-cli`です。インストールすると`tokens`コマンドが直接`PATH`に追加され、現在のプラットフォーム向けネイティブバイナリが自動的に選択されます。
...
> **注**: `bun run cli`はローカル開発用です。`bunx tokens-cli`ではコマンドが直接実行されます。以下の使用法セクションはインストールされたバイナリコマンドを示しています。
...
> **注**: `bunx tokens-cli@latest`でインストールすると、ネイティブバイナリはビルド済みで含まれています。ソースからのビルドはローカル開発にのみ必要です。
...
**仕組み**: `tokens antigravity sync` はローカルの Antigravity セッション候補を検出し、ローカル言語サーバ RPC から確定済みの使用量データを取得して、tokscale-core が後で解析できるよう正規化された JSONL アーティファクトとして保存します。最新の Antigravity データをレポートに反映したい場合は、レポート実行前に sync を実行してください。
...
再生成可能な CLI/TUI/料金/Wrapped キャッシュは `~/.config/tokens/cache/` 配下に保存されます（`TOKENS_CONFIG_DIR` を設定した場合は `${TOKENS_CONFIG_DIR}/cache/`）。連携同期アーティファクトは `~/.config/tokens/antigravity-cache/` や `~/.config/tokens/trae-cache/` など、クライアントごとのキャッシュルートに保存されます。
...
| 変数 | デフォルト | 説明 |
|----------|---------|-------------|
| `TOKENS_NATIVE_TIMEOUT_MS` | `300000`（5分） | `nativeTimeoutMs` 設定をオーバーライド |
| `TOKENS_CONFIG_DIR` | unset | 設定ディレクトリのルート（`settings.json`、`star-cache.json`、`cache/`、`antigravity-cache/`、`trae-cache/` の保存場所）をオーバーライドします。絶対パス推奨；相対パスはプロセス CWD を基準に解決されます。CI サンドボックスや非デフォルトの場所を固定したい場合に便利です。設定されている場合、tokens は macOS のレガシーパス（`~/Library/Application Support/tokens/`）にフォールバックしません。 |
...
> **注**: 恒久的な変更には、`~/.config/tokens/settings.json`で`nativeTimeoutMs`を設定することをお勧めします。環境変数は一時的なオーバーライドやCI/CDに適しています。
...
macOSでは、\`TOKENS_HEADLESS_DIR\`が設定されていない場合、Tokensは\`~/Library/Application Support/tokens/headless/\`もスキャンします。
...
**環境変数:** \`TOKENS_HEADLESS_DIR\`を設定してヘッドレスログディレクトリをカスタマイズできます:
```bash
...
1. **ログイン** - `tokens login`を実行してGitHubで認証
...
| Cursor | API同期 | API同期 | APIでデータを取得、`%USERPROFILE%\.config\tokens\cursor-cache\`にキャッシュ |
...
- **TUI設定**: `%APPDATA%\tokens\settings.json`（プラットフォームのデフォルト。`TOKENS_CONFIG_DIR` でオーバーライド可能）
