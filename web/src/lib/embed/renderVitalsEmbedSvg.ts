...
export function renderVitalsEmbedSvg(
  ...
  const H = BASE_H + (options.today ? 26 : 0);

  return `<?xml version="1.0" encoding="UTF-8"?>
<svg data-template="vitals" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Tokens usage signals for @${escapeXml(data.user.username)}">
  ${cardTextStyle()}
