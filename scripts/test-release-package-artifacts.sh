...
TMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/tokens-release-artifact-smoke.XXXXXX")"
cleanup() {
  rm -rf "${TMP_ROOT}"
}
trap cleanup EXIT
...
const rows = [
  ["cli-darwin-x64", "cli-binary-x86_64-apple-darwin", "tokens"],
  ["cli-darwin-arm64", "cli-binary-aarch64-apple-darwin", "tokens"],
  ["cli-linux-x64-gnu", "cli-binary-x86_64-unknown-linux-gnu", "tokens"],
  ["cli-linux-x64-musl", "cli-binary-x86_64-unknown-linux-musl", "tokens"],
  ["cli-linux-arm64-gnu", "cli-binary-aarch64-unknown-linux-gnu", "tokens"],
  ["cli-linux-arm64-musl", "cli-binary-aarch64-unknown-linux-musl", "tokens"],
  ["cli-win32-x64-msvc", "cli-binary-x86_64-pc-windows-msvc", "tokens.exe"],
  ["cli-win32-arm64-msvc", "cli-binary-aarch64-pc-windows-msvc", "tokens.exe"],
];
...
echo "Building tokens-cli JavaScript entrypoint..."
...
echo "Installing tokens-cli release tarball with Bun..."
(
  cd "${INSTALL_DIR}"
  env PATH="${BUN_ONLY_DIR}" bun add "${CLI_STAGE}/${CLI_TGZ}" >/dev/null
)

CLI_BIN="${INSTALL_DIR}/node_modules/tokens-cli/bin.js"
HOST_BINARY="${INSTALL_DIR}/node_modules/tokens-${HOST_PACKAGE}/bin/${HOST_BINARY_NAME}"
[[ -f "${CLI_BIN}" ]] || fail "Installed tokens-cli bin.js missing"
