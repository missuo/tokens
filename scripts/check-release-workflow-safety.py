...
def main() -> None:
    ...
    if set(publish_build) != set(TARGET_PACKAGES):
        errors.append(
            f"build matrix targets differ from supported targets: publish={sorted(publish_build)}, expected={sorted(TARGET_PACKAGES)}"
        )

    for target, publish_entry in publish_build.items():
        expected_package_dir = TARGET_PACKAGES.get(target)
        if expected_package_dir is None:
            continue
        ...
        expected_binary = "tokens.exe" if target.endswith("windows-msvc") else "tokens"
        if publish_entry.get("bin_name") != expected_binary:
            errors.append(
                f"build matrix {target} binary drift: expected {expected_binary}, found {publish_entry.get('bin_name')}"
            )
