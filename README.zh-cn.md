...
> **运行 [`bunx tokens submit`](#社交平台命令) 将您的使用数据提交到排行榜并创建公开个人资料！**

## 概述

**Tokens** 帮助您监控和分析以下平台的 Token 消耗：
...
| <img width="48px" src=".github/assets/client-cursor.jpg" alt="Cursor" /> | [Cursor IDE](https://cursor.com/) | Cursor API 导出缓存于 `~/.config/tokens/cursor-cache/usage*.csv`（而非 `~/.cursor`） | ✅ 支持 |
...
| <img width="48px" src=".github/assets/client-antigravity.png" alt="Antigravity" /> | [Google Antigravity](https://antigravity.google/) | 通过 `tokens antigravity sync` 缓存到 `~/.config/tokens/antigravity-cache/sessions/*.jsonl`（使用本地语言服务器 RPC） | ✅ 支持 |
...
| <img width="48px" src=".github/assets/client-trae.png" alt="Trae" /> | [Trae IDE](https://www.trae.ai/) / [Trae Solo](https://www.trae.ai/solo)（国际版） | 通过 `tokens trae sync` 缓存到 `~/.config/tokens/trae-cache/sessions/*.json`（来自官方 API 的账号级使用量） | ✅ 支持 |
| <img width="48px" src="https://github.com/warpdotdev.png" alt="Warp" /> | [Warp](https://www.warp.dev/) / Oz | 通过 `tokens warp sync` 缓存到 `~/.config/tokens/warp-cache/usage.json`（仅汇总请求数和消费金额，不含 token 转录） | ✅ 支持 |
...
在 AI 辅助开发的时代，**Token 就是新的能源**。它们驱动我们的思考，提升我们的生产力，推动我们的创造性产出。正如卡尔达肖夫指数在宇宙尺度上追踪能源消耗，Tokens 在您攀登 AI 增强开发的阶梯时测量您的 Token 消耗。无论您是休闲用户还是每天消耗数百万 Token，Tokens 都能帮助您可视化从行星级开发者到银河级代码架构师的旅程。

## 目录

- [概述](#概述)
  - [为什么叫 "Tokens"？](#为什么叫-tokens)
- [功能](#功能)
- [安装](#安装)
  - [快速开始](#快速开始)
  - [先决条件](#先决条件)
...
> **包结构**：`tokens` 是一个别名包（类似 [`swc`](https://www.npmjs.com/package/swc)），它安装 `@tokens/cli`。两者都安装包含原生 Rust 核心（`@tokens/core`）的相同 CLI。
...
> **注意**：`bun run cli` 用于本地开发。通过 `bunx tokens` 安装后，命令直接运行。下面的使用部分显示已安装的二进制命令。
...
> **注意**：通过 `bunx tokens@latest` 安装时，原生二进制文件已预构建并包含在内。仅在本地开发时才需要从源码构建。
...
tokens --json                    # 默认模型视图为 JSON
tokens models --json             # 模型分解为 JSON
tokens monthly --json            # 月度分解为 JSON
tokens models --json > report.json   # 保存到文件
```
...
- **设置持久化**：偏好设置保存到 `~/.config/tokens/settings.json`（参见[配置](#配置)）
...
`tokens models --json --group-by session,model` 会为每个 `(session_id, model)` 输出一个条目。每个条目都包含顶层的 `sessionId` 字段，以便下游工具（例如多代理 IDE）能将成本数据关联回特定的 agent-CLI 会话：
...
# Cursor IDE 需要先运行 `tokens cursor login`
...
> **破坏性变更（v4.0.0）**：单客户端布尔选项（`--opencode`、`--claude`、`--codex` 等）已被移除，现在会直接报错。请改用规范的 `--client`/`-c` 选项——例如 `tokens --client opencode,claude`。

### 日期筛选

日期筛选器适用于所有生成报告的命令（`tokens`、`tokens models`、`tokens monthly`、`tokens graph`）：

```bash
# 快速日期快捷方式
tokens --today              # 仅今天
tokens --yesterday          # 仅昨天
tokens --week               # 最近 7 天
tokens --month              # 本月
...
1. **自定义价格覆盖** - 来自 `~/.config/tokens/custom-pricing.json` 的用户自定义精确条目
...
在 Tokens 的配置目录中创建 `custom-pricing.json`（macOS/Linux 上默认为 `~/.config/tokens/custom-pricing.json`；若设置了 `TOKSCALE_CONFIG_DIR`，则为其解析出的同一目录），以覆盖上游价格数据库尚未正确收录的模型 ID 的价格。
...
覆盖价格以每百万 Token 的美元数输入，这与大多数 API 提供商公布价格的方式一致；Tokens 会在内部将其转换为每 Token 的费率。`input_cost_per_million_tokens` 或 `output_cost_per_million_tokens` 中至少要有一个存在且为正值，缓存读取/缓存创建字段为可选。为兼容复制粘贴，也接受 LiteLLM 风格的每 Token 字段名，例如 `input_cost_per_token`、`output_cost_per_token` 和 `cache_read_input_token_cost`，但推荐面向用户使用每百万的命名形式。要省略某个层级或缓存价格，直接不写该字段即可；负值或非有限值会被视为无效，并跳过整个模型条目，以免拼写错误悄悄改变统计。可选的 `source` 和 `notes` 字段会被 Tokens 忽略，可用于您自己的记账。

覆盖是仅精确匹配且不区分大小写的。Tokens 先检查原始模型 ID，再检查现有的合成 `/models/` 归一化，然后才在没有覆盖匹配时回退到 LiteLLM、OpenRouter、Cursor 定价和模糊匹配。原始精确匹配优先于归一化精确匹配，因此 `accounts/fireworks/routers/kimi-k2p6-turbo` 可以覆盖某个特定网关的模型，而 `kimi-k2p6-turbo` 可以覆盖归一化的 `/models/` 路径。覆盖在启动时仅加载一次；编辑文件后请重启命令。这是在等待上游 LiteLLM 价格更新期间，针对错误模型定价 Bug 的推荐本地修复方案。
...
```bash
# 登录 Tokens（打开浏览器进行 GitHub 认证）
tokens login

# 无需浏览器认证即可保存已有的 Tokens API token
...
# 在 CI/无头环境中提交，且不写入凭据
# 优先级：TOKSCALE_API_TOKEN 环境变量 > 已保存的凭据文件（~/.config/tokens/credentials.json）。
# 设置了该环境变量时，本次调用会忽略已保存的文件。
TOKSCALE_API_TOKEN=tt_xxx tokens submit

# 撤销 token：访问排行榜站点的 Settings > API Tokens
#（https://tokens.ci/settings），点击对应 token 行的 "Revoke"。
...
**凭据存储**：Cursor 账户保存到 `~/.config/tokens/cursor-credentials.json`。使用量数据缓存在 `~/.config/tokens/cursor-cache/`（活动账户使用 `usage.csv`，其他账户使用 `usage.<account>.csv`）。

默认情况下，tokens 会 **合并统计所有已保存 Cursor 账户的使用量**（`cursor-cache/usage*.csv`）。为保持兼容性，活动账户会同步到 `cursor-cache/usage.csv`。

登出时，tokens 会将缓存的历史记录移动到 `cursor-cache/archive/`（因此不会参与合并统计）。如需彻底删除缓存，请使用 `--purge-cache`。
...
Antigravity 同步目前仅在 macOS 和 Linux 上受支持。只有在启用了 Antigravity 的编辑器正在运行且本地语言服务器可用时才生效；tokens 会从该本地语言服务器读取使用量，并将归一化后的产物缓存到本地。

```bash
# 检查 tokens 是否能识别正在运行的 Antigravity 语言服务器
tokens antigravity status

# 将本地 Antigravity 语言服务器中的使用量同步到 tokens 的缓存
...
**缓存位置**：`~/.config/tokens/antigravity-cache/`

**工作原理**：`tokens antigravity sync` 会发现本地的 Antigravity 会话候选项，从本地语言服务器 RPC 获取已确认的使用数据，并以归一化的 JSONL 产物形式存储，供 tokens-core 后续解析。如果希望报告反映最新的 Antigravity 数据，请在生成报告前先运行同步。

### Trae 命令

Trae（[ByteDance 的 AI IDE](https://www.trae.ai/)）有两个国际版产品线 —— Trae IDE 和 Trae Solo。它们共享同一份账号级使用量数据（同后端、同 JWT），所以 tokens 将其统一报告为一个 `trae` 客户端。装一个或两个都可以，tokens 会自动从任何已安装的桌面端发现凭据。
...
`tokens trae sync` 每次运行只调用一次官方的 `query_user_usage_group_by_session` API（无论安装了几个桌面端），并将原始 JSON 持久化到本地缓存。
...
**缓存位置**：`~/.config/tokens/trae-cache/`

**工作原理**：tokens 会解密桌面客户端的 `iCubeAuthInfo://*` blob（`globalStorage/storage.json`）来恢复 JWT，或接受通过 `--manual` 粘贴的 JWT。随后它分页调用 `POST /trae/api/v1/pay/query_user_usage_group_by_session` 并保存原始 JSON。如果希望报告反映最新的 Trae 数据，请在生成报告前先运行同步。

> **中国区版本**：中国区版本（`trae.com.cn`）目前有意不支持。CN 后端暂未暴露按会话查询使用量的官方 API；如果上游提供正式端点，再加入支持。

### Warp/Oz 命令

Warp/Oz 不提供本地 token 转录。Tokens 仅同步 Warp GraphQL API 返回的汇总请求数和消费计数器，并将其以 `warp` / `aggregate-requests` 行（token 字段均为零）的形式上报。
...
**缓存位置**：`~/.config/tokens/warp-cache/usage.json`

**工作原理**：`tokens warp sync` 调用 Warp 已认证的 GraphQL API，获取账号和工作区的汇总计数器。Tokens 将请求数保留为消息计数，将供应商上报的消费金额保留为成本，但不会将请求数转换为合成 token。由于公开排行榜只接受基于 token 归因的使用量，Warp 数据默认不包含在 `submit` 提交内容中。
...
1. 扫描会话并将其插入本地 SQLite wiki 数据库（`wiki.db`，位于平台配置目录——Linux 上为 `~/.config/tokens/`，macOS 上为 `~/Library/Application Support/tokens/`）
2. 未摘要的会话分批发送到选定的 LLM 后端，后端为每个会话返回标题、类别、描述和复杂度
3. 第二轮 LLM 处理将所有已加标题的会话归并为 3–8 个高层级任务集群（例如 "Kiro Auth"、"Tokens Report"、"System Config"）
...
Tokens 可以获取并显示您在各 AI 提供商上的实时订阅配额。它会显示您已使用了多少套餐额度，以及限额何时重置。
...
> **注意**：订阅配额和余额均为**供应商上报**——tokens 调用每个提供商自己的配额端点，并原样呈现其返回结果。这些数字反映的是提供商上报的值（也就是其官方仪表板上显示的值），并未与 tokens 自身的使用量跟踪进行独立核对。
...
| **Codex**（OpenAI） | OAuth（`~/.config/codex/auth.json`、`~/.codex/auth.json`，或已保存的 Tokens 账号） | Session、Weekly 配额 | 在 TUI Usage 标签中使用 `[Add Codex]`，运行 `codex` 登录，或用 `tokens codex import --name work` 导入现有认证 |
...
Tokens 可以保存多个 Codex OAuth 账号用于订阅使用量显示。TUI Usage 标签会将已保存的账号归并到一个 **Codex** 区块下。活动账号以 `*` 标记；非活动账号可通过 `[Use]` 选中；移除账号使用 `[Remove]` 后接 `[Confirm]`。

要在不离开 TUI 的情况下添加账号，请在 Usage 标签中点击 `[Add Codex]`。Tokens 会用一个临时的 `CODEX_HOME` 启动 `codex login`，在 Usage 标签中显示登录输出，将生成的认证导入 Tokens 的账号存储，然后刷新使用量。这样可保持登录隔离，且不会切换当前的 Codex 认证；当您希望 Tokens 将某个已保存账号写入真正的 Codex 认证文件时，点击该账号上的 `[Use]`。

CLI 命令仍然可用于脚本化或手动的账号管理：

```bash
# 将当前 Codex 认证保存为命名的 Tokens 账号
...
# 停止跟踪某个已保存的 Codex 账号（仅从 Tokens 的存储中移除——
...
当存在已保存的 Codex 账号时，`tokens usage --json` 会为每个 Codex 条目包含结构化的账号元数据，TUI 会将这些条目显示在一个 Codex 分组下。若无已保存的账号，Tokens 会回退到当前的 Codex 认证发现路径（`CODEX_HOME/auth.json`、`~/.config/codex/auth.json`、`~/.codex/auth.json`，然后是 macOS 钥匙串）。
...
Tokens 将设置存储在 `~/.config/tokens/settings.json`：
...
| `light.writeCache` | boolean | `false` | 为 `true` 时，`tokens --light` 会在渲染完成后以原子方式覆盖 TUI 缓存。CLI 标志 `--write-cache` / `--no-write-cache` 会按次运行覆盖该设置。 |
...
| `scanner.extraScanPaths` | object | `{}` | 针对 Tokens 默认 home 根位置之外的会话，为各客户端额外指定的扫描根目录 |

使用 `scanner.extraScanPaths` 配置持久化的额外根目录，例如项目级的 `.codex` 目录、导入的 Gemini/OpenClaw 历史，或 Hermes 配置文件数据库。Hermes 条目既可以指向包含 `state.db` 的配置文件目录，也可以直接指向 `state.db` 文件。Tokens 在每次运行时都会将这些路径与默认扫描根目录合并，并按规范路径去重重叠的根目录。

使用 `defaultClients` 固定一个个人默认值 —— 例如，如果您只使用 OpenCode 和 Claude，就将其设为 `["opencode", "claude"]`，那么 `tokens`（不带任何选项）会自动将每个报告的范围限定为它们。在命令行传入 `--client` 可针对单次运行进行覆盖。
...
要启用它，请在 `~/.config/tokens/settings.json` 中将 `minutelyTabEnabled` 设为 `true`：
...
可再生成的 CLI/TUI/价格/Wrapped 缓存位于 `~/.config/tokens/cache/` 下（如果设置了 `TOKSCALE_CONFIG_DIR`，则为 `${TOKSCALE_CONFIG_DIR}/cache/`）。集成同步产物保留在各自的客户端缓存目录中，例如 `~/.config/tokens/antigravity-cache/` 和 `~/.config/tokens/trae-cache/`：
...
删除该目录是安全的。Tokens 会在需要时重新创建并重新生成其中的内容。
...
| `TOKSCALE_API_TOKEN` | unset | 用于非交互式 `submit` 和 `delete-submitted-data` 运行的 Tokens 个人 API token。可从 Settings > API Tokens 创建一个，或用 `tokens login --token tt_xxx` 保存到本地。 |
| `TOKSCALE_EXTRA_DIRS` | unset | 一次性的额外会话根目录，格式为 `client:/abs/path,client:/abs/path` |
| `TOKSCALE_CONFIG_DIR` | unset | 覆盖配置目录根（`settings.json`、`star-cache.json`、`cache/`、`antigravity-cache/`、`trae-cache/` 的存放位置）。建议使用绝对路径；相对路径将基于进程 CWD 解析。适用于 CI 沙箱或固定到非默认位置。设置后，tokens 不会回退到 macOS 旧路径（`~/Library/Application Support/tokens/`）。 |
...
> **注意**：对于持久化的额外根目录，建议在 `~/.config/tokens/settings.json` 中使用 `scanner.extraScanPaths`。`TOKSCALE_EXTRA_DIRS` 最适合一次性覆盖或 CI/CD。

### Headless 模式

Tokens 可以聚合来自 **Codex CLI 无头输出**的令牌使用情况，用于自动化、CI/CD 流水线和批处理。

**什么是 Headless 模式？**

当您使用 JSON 输出标志运行 Codex CLI 时（例如 \`codex exec --json\`），它会将使用数据输出到 stdout，而不是存储在常规会话目录中。Headless 模式允许您捕获和跟踪这些使用情况。

**存储位置：** \`~/.config/tokens/headless/\`

在 macOS 上，当未设置 \`TOKSCALE_HEADLESS_DIR\` 时，Tokens 也会扫描 \`~/Library/Application Support/tokens/headless/\`。
...
Tokens 包含一个社交平台，您可以在其中分享使用数据并与其他开发者竞争。
...
您可以直接在 GitHub 个人资料 README 中嵌入 Tokens 公开统计数据：
...
1. **登录** - 运行 `tokens login` 通过 GitHub 认证，或在 Settings 中创建一个 API token 供 CI/无头环境使用
2. **提交** - 运行 `tokens submit` 上传使用数据
...
```bash
tokens --benchmark           # 显示默认视图的处理时间
tokens models --benchmark    # 基准测试模型报告
tokens monthly --benchmark   # 基准测试月度报告
tokens graph --benchmark     # 基准测试图表生成
```
...
Tokens 完全支持 Windows。TUI 和 CLI 的工作方式与 macOS/Linux 相同。
...
| Cursor | API 同步 | API 同步 | 通过 API 获取数据，缓存在 `%USERPROFILE%\.config\tokens\cursor-cache\` |
...
| Antigravity | `~/.config/tokens/antigravity-cache/sessions/` | — | `tokens antigravity sync` 目前仅支持 macOS / Linux |
...
| Trae | `~/.config/tokens/trae-cache/sessions/` | `%APPDATA%\tokens\trae-cache\sessions\` | 通过 `tokens trae sync` 同步一次；凭据会从已安装的任意 Trae IDE 或 Trae Solo 桌面端自动发现 |
| Warp/Oz | `~/.config/tokens/warp-cache/usage.json` | `%APPDATA%\tokens\warp-cache\usage.json` | 通过 `tokens warp sync` 同步；仅汇总请求数和消费金额，不含 token 转录 |
...
Tokens 将配置存储在：
- **TUI 设置**: `%APPDATA%\tokens\settings.json`（平台默认值。可用 `TOKSCALE_CONFIG_DIR` 覆盖）
- **缓存**: `%APPDATA%\tokens\cache\`（统一缓存根目录）
- **旧版缓存路径**: 旧版本曾使用 `%USERPROFILE%\.cache\tokens\` 这类分散路径；在可再生数据写入新路径之前，这些旧路径可能仍然存在。
- **Cursor 凭据**: `%USERPROFILE%\.config\tokens\cursor-credentials.json`
- **Trae 凭据和同步使用量**: `%APPDATA%\tokens\trae-cache\`
- **Tokens 账号凭据**: `%USERPROFILE%\.config\tokens\credentials.json`
...
OpenCode 1.2+ 将会话存储在 SQLite 中。Tokens 优先从 SQLite 读取，旧版本则回退到旧版 JSON 文件。

OpenCode 根据构建时的发布渠道决定数据库文件名：`latest`/`beta` 渠道使用 `opencode.db`，其他渠道使用 `opencode-<channel>.db`（例如 `opencode-stable.db`、`opencode-nightly.db`）。Tokens 会扫描所有这些文件，因此同时使用多个渠道的用户也能获得统一的视图。

如果您用 `OPENCODE_DB` 指向 `~/.local/share/opencode` 之外的文件来启动 opencode，请将该绝对路径添加到 `~/.config/tokens/settings.json`，以便 tokens 每次运行都能找到它：
...
如果您将会话保存在 Tokens 默认 home 根位置之外，也可以为各客户端持久化额外的扫描根目录：
...
这对于项目级的 `.codex` 目录、导入的历史，以及默认 `$HERMES_HOME/state.db` 或 `~/.hermes/state.db` 位置之外的 Hermes 配置文件数据库都很有用。Tokens 仍会扫描其默认根目录，然后在其上合并 `scanner.extraScanPaths` 和 `TOKSCALE_EXTRA_DIRS`，并按规范路径去重。它不会自动发现您的整个工作区。
...
Tokens 的 `claude` 客户端统计的是 Claude Code 的 Token，而非 Claude Desktop 聊天。Claude Desktop 会将应用数据存储在 `~/Library/Application Support/Claude` 等位置，但 Anthropic 并未为消费级桌面聊天或聊天历史导出提供文档化、稳定的本地按消息 Token 账本。当存在 Claude Desktop 数据但只有 Claude Code JSONL 根目录可扫描时，运行 `tokens clients` 可看到一条诊断信息。`tokens usage` 可以根据 Claude Code 凭据尽力显示 Claude 订阅配额条，而组织/API 使用量属于 Anthropic 的 Admin Usage 和 Cost API，与本地转录扫描有意分离。
...
Tokens 将 `chat` span 作为 Token 统计的真实来源，并在第一阶段忽略工具 span 和累积指标：
...
> Copilot 的 OTEL 负载目前不暴露稳定的工作区元数据，因此 Copilot 行可能不会显示工作区属性。Tokens 尽可能根据报告的模型为这些行定价，不会直接信任 `github.copilot.cost`。
...
位置：`~/.config/tokens/cursor-cache/usage*.csv`（通过 Cursor API 同步）

Cursor 数据使用您的会话令牌从 Cursor API 获取并本地缓存。Tokens 读取这些缓存文件来生成报告；它不会解析本地的 `~/.cursor` 会话数据。设置说明请参阅 [Cursor IDE 命令](#cursor-ide-命令)。

### Antigravity

位置：`~/.config/tokens/antigravity-cache/sessions/*.jsonl`（通过本地 Antigravity 语言服务器 RPC 同步）

Antigravity 数据不会被根命令自动获取。请在启用了 Antigravity 的编辑器打开时运行 `tokens antigravity sync` 来刷新本地缓存，然后对缓存的 JSONL 产物使用常规的 tokens 报告和筛选。

### Trae

位置：`~/.config/tokens/trae-cache/sessions/*.json`（通过官方使用量 API 同步）

Trae 数据不会被根命令自动获取。先运行一次 `tokens trae login`，然后在生成报告前运行 `tokens trae sync`。Tokens 会将同步得到的 API dump 解析为会话级记录，并保留 Trae 返回的成本总额。

### Warp/Oz

位置：`~/.config/tokens/warp-cache/usage.json`（通过已认证的 GraphQL API 同步）

Warp/Oz 数据不会被根命令自动获取。先运行 `tokens warp login`，然后在生成报告前运行 `tokens warp sync`。由于 Warp 不暴露基于 token 归因的本地转录，Tokens 仅记录汇总的请求数和消费金额。

### Grok Build

位置：`$GROK_HOME/sessions/*/*/updates.jsonl`（回退：`~/.grok/sessions/*/*/updates.jsonl`）

Grok Build 数据直接从本地会话更新解析。当前日志只公开累积 `totalTokens` 计数器，没有稳定的 input/output 拆分，因此 Tokens 将每个 turn 的正向增量记录为 input token。`grok-composer-2.5-fast` 会临时映射到 Composer 2.5 Fast 价格 override，直到专用公开价格可用。

### Jcode

位置：`$JCODE_HOME/sessions/session_*.json`（回退：`~/.jcode/sessions/session_*.json`）以及匹配的 `session_*.journal.jsonl` sidecar。

Jcode 数据直接从本地会话快照解析。Tokens 读取助手消息的 `messages[].token_usage` 字段（`input_tokens`、`output_tokens`、`cache_read_input_tokens`、`cache_creation_input_tokens` 和 `reasoning_output_tokens`），不会伪造其他客户端的身份。匹配的 journal sidecar 会在去重前合并进同一会话流，因此在 Jcode 将其检查点写入快照之前，最近追加的消息也会被包含进来。去重使用稳定的消息 ID 进行重放去重；缺少 ID 的畸形/自定义记录则使用作用域内的回退 key。
...
Hermes 将会话级使用量存储在 SQLite `sessions` 表中。Tokens 导入 `model` 存在且 token 或费用合计非零的行，使用 `started_at` 作为时间戳，保留 `message_count`，并优先使用 `actual_cost_usd` 而非 `estimated_cost_usd`。
...
`ui_messages.json` 是一个 UI 事件数组。Tokens 仅计算：
- `type == "say"`
- `say == "api_req_started"`
...
Kilo 使用与 Roo Code 相同的任务日志格式。Tokens 应用相同的规则：
...
Mux 在 `session-usage.json` 文件中存储每个会话的累计 Token 使用量。每个文件包含一个 `byModel` 映射，其中包含每个模型的 Token 细分：
 ...
 模型名称使用 `provider:model` 格式（例如 `anthropic:claude-opus-4-6`）— tokens 会去除 provider 前缀以识别模型
...
Crush 在项目级 SQLite 数据库（`crush.db`）中存储使用量。由于 Crush 不提供可靠的每条消息或每个模型的 Token 统计，Tokens 仅导入根会话的会话级成本总计。记录显示为 `model=session-total`，Token 细分为零。
...
Goose 在 SQLite 的 `sessions.db` 中按会话保存使用量。Tokens 从 `model_config_json` 提取模型，从 `provider_name` 提取提供商，并按会话累加输入/输出 Token 总数。当推理 Token 列有数据时也会被推断出来。
...
Codebuff（前身 Manicode）按聊天写入 JSON 文件。Tokens 从 `metadata.usage`、`metadata.codebuff.usage` 以及 run-state 中的 `messageHistory[*].providerOptions` 回退路径解析 Token 使用量，并以反向顺序遍历历史，避免部分更新的较新条目遮蔽实际承载 Token 数的较早条目。当消息时间戳缺失时，会回退到 chat-id 目录名，最后回退到文件 mtime。
...
包含会话头和消息条目的 JSONL 格式。Tokens 仅处理 assistant 消息，并在存在时优先使用 gjc 的权威性每条消息 `usage.cost.total`（USD），仅在缺失时才从 token 数重新计算：
```json
...
Tokens 还会检测 `~/.local/share/octofriend/sqlite.db`，并在可用时解析包含 token 数据的记录。

### MiMo Code

位置：`~/.local/share/mimocode/mimocode.db`（XDG 数据目录）

MiMo Code 将会话数据存储在 SQLite 数据库中。Tokens 查询 `message` 表并关联 `session` 表获取工作区上下文：
...
Tokens 使用时间戳、模型、provider、token 计数、成本和 agent 名称的指纹对跨 fork 会话的消息进行去重。

## 定价

Tokens 从 [LiteLLM 的价格数据库](https://github.com/BerriAI/litellm/blob/main/model_prices_and_context_window.json)获取实时价格。

**动态回退**：对于 LiteLLM 中尚未收录的模型（例如最近发布的模型），Tokens 会自动从 [OpenRouter 的端点 API](https://openrouter.ai/docs/api/api-reference/endpoints/list-endpoints) 获取定价。这确保您无需等待 LiteLLM 更新，就能从模型的作者提供商（例如 glm-4.7 的 Z.AI）获得准确的价格。

**Cursor 模型定价**：对于 LiteLLM 和 OpenRouter 中都尚未收录的最新模型（例如 `gpt-5.3-codex`），Tokens 使用从 [Cursor 模型文档](https://cursor.com/en-US/docs/models)获取的硬编码定价。这些覆盖在所有上游来源之后、模糊匹配之前检查，因此当真正的上游定价可用时会自动让步。
...
- LiteLLM 缓存：`~/.config/tokens/cache/pricing-litellm.json`
- OpenRouter 缓存：`~/.config/tokens/cache/pricing-openrouter.json`（缓存支持提供商的模型作者定价信息）
