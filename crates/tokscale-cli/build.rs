...
//! final binary.
//!
//! The dylib is deliberately NOT linked into `tokens`. Apple's
...
//!
//! Instead the binary links nothing FM/Swift (verifiable: `otool -L tokens`
...
fn build_apple_fm() {
    ...
    println!("cargo:rustc-env=TOKENS_FM_DYLIB={}", out_lib.display());
    ...
    //    shipped npm package, where the dylib travels alongside `tokens`.
    //
    ...
    if let Some(profile_dir) = profile_dir_from_out(&out_dir) {
        ...
        // bin/ next to tokens; surface its path for that step / debugging.
