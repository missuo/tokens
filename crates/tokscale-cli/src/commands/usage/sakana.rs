...
/// so `TOKENS_CONFIG_DIR` / XDG overrides are honored (matching every other
/// provider, e.g. `codex.rs`), instead of hardcoding `~/.config/tokens`.
///
    ...
    let cookie = session_cookie().ok_or_else(|| {
        anyhow::anyhow!(
            "No Sakana session cookie. Set SAKANA_SESSION_COOKIE or write a \
             `sakana-session` file in the tokens config dir."
        )
    })?;
...
"#;
    ...
    // dir, which honors `TOKENS_CONFIG_DIR`, instead of hardcoding
    // `~/.config/tokens`. Serial because it mutates process-global env.
    #[test]
    #[serial_test::serial]
    fn session_cookie_reads_from_overridden_config_dir() {
        use std::env;

        let prev_dir = env::var_os("TOKENS_CONFIG_DIR");
        ...
        unsafe {
            env::set_var("TOKENS_CONFIG_DIR", &tmp);
