...
mod tests {
    ...
    fn cron_block_replacement_preserves_unrelated_jobs() {
        let existing =
            "0 0 * * * echo keep\n# BEGIN TOKENS AUTOSUBMIT\nold\n# END TOKENS AUTOSUBMIT\n";
    ...
    fn launchd_spec_uses_program_arguments_without_shell() {
        ...
        let spec = render_launchd_spec(Path::new("/usr/local/bin/tokens"), &settings).unwrap();
        let content = &spec.files[0].1;
        assert!(content.contains("<string>/usr/local/bin/tokens</string>"));
    ...
    fn systemd_spec_uses_autosubmit_run() {
        ...
        let spec = render_systemd_spec(Path::new("/usr/local/bin/tokens"), &settings).unwrap();
        let service = &spec.files[0].1;
        let timer = &spec.files[1].1;
        assert!(service.contains("ExecStart=/usr/local/bin/tokens autosubmit run"));
    ...
    fn systemd_spec_honors_xdg_config_home() {
        ...
        assert_eq!(
            ...
            temp.path()
                .join("systemd")
                .join("user")
                .join("tokens-autosubmit.service")
        );
        assert_eq!(
            spec.files[1].0,
            temp.path()
    ...
    fn windows_spec_uses_fixed_task_name() {
        ...
        let spec = render_windows_task_spec(Path::new("C:/bin/tokens.exe"), &settings).unwrap();
    ...
    fn windows_spec_rejects_long_non_day_interval() {
        ...
        let err = render_windows_task_spec(Path::new("C:/bin/tokens.exe"), &settings)
    ...
    fn run_lock_blocks_concurrent_holder() {
        let temp = TempDir::new().unwrap();
        let _guard = EnvVarGuard::set("TOKENS_CONFIG_DIR", temp.path());
