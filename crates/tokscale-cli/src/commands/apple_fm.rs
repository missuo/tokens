...
//! the `tokens` binary hard-linked any of them it would fail to `dyld`-load on
...
//!
//! So the binary links nothing FM/Swift (verifiable: `otool -L tokens` shows
...
mod imp {
    ...
    fn candidate_paths() -> Vec<PathBuf> {
        ...
        if let Some(p) = option_env!("TOKENS_FM_DYLIB") {
    ...
    fn load_api() -> Option<Api> {
        // Set `TOKENS_FM_DEBUG=1` to trace why apple-fm did/didn't engage
        // (OS gate, which dylib path loaded, dlopen errors, symbol resolution).
        let debug = std::env::var_os("TOKENS_FM_DEBUG").is_some();
...
mod tests {
    ...
    fn project_name_and_title_from_workspace() {
        let s = heuristic_classify(&input(0, 0, "/Users/x/tokens", &["claude-opus-4"]));
        assert_eq!(s.title, "Work on tokens");
        assert_eq!(s.task_category, "other");
        assert_eq!(s.description, "Session in tokens using claude-opus-4.");
    }
