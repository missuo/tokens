...
pub fn max_rpc_body_bytes() -> usize {
    std::env::var("TOKENS_ANTIGRAVITY_MAX_RPC_BODY_BYTES")
        .ok()
    ...
    fn acquire(cache_dir: &Path, operation: &str) -> Result<Self> {
        ...
        match file.try_lock_exclusive() {
            ...
            Err(err) if crate::commands::autosubmit::is_lock_contention(&err) => {
                anyhow::bail!("Another tokens {operation} is in progress; aborting")
            }
        ...
        match os_file.try_lock_exclusive() {
            ...
            Err(err) if crate::commands::autosubmit::is_lock_contention(&err) => {
                ...
                let owner = read_sync_lock(&lock_path)
                    ...
                    .unwrap_or_default();
                anyhow::bail!("Another tokens antigravity sync is in progress{owner}; aborting");
            }
            Err(err) => {
                return Err(
...
fn existing_sync_lock_error(lock_path: &Path) -> anyhow::Error {
    if let Some((pid, _)) = read_sync_lock(lock_path).filter(|(pid, _)| pid_is_alive(*pid)) {
        anyhow::anyhow!(
            "Another tokens Antigravity sync may be in progress (pid {pid}); do not remove '{}' until that process has stopped. If it has stopped, remove '{}' and retry.",
            lock_path.display(),
            lock_path.display()
        )
    } else {
        anyhow::anyhow!(
            "Antigravity sync lock at '{}' already exists. To avoid overlapping a possible active sync during a rolling upgrade, tokens will not replace it automatically. Confirm no tokens Antigravity sync is running, then remove '{}' and retry.",
            lock_path.display(),
            lock_path.display()
        )
    }
...
/// read, so skipping it would loop forever. tokens writes these artifacts
...
fn atomic_write_file(path: &Path, contents: &str) -> Result<()> {
    ...
    if let Err(err) = tokens_core::fs_atomic::replace_file(&temp_path, path) {
        let _ = fs::remove_file(&temp_path);
...
fn https_rpc_request(
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // does not depend on; until then the environment tokens is launched
        // with is trusted.
    ...
    #[cfg(not(target_os = "windows"))]
    {
        ...
        // context entered (#1264): the TUI usage refresh and `tokens
...
fn antigravity_https_client() -> &'static reqwest::Client {
    HTTPS_RPC_CLIENT.get_or_init(|| {
        tokens_core::http::client_builder()
...
mod tests {
    ...
    /// `$XDG_CONFIG_HOME` first; tokens's own `paths::get_config_dir()`
...
"#;
    ...
    fn rpc_request_rejects_oversized_content_length_body() {
        let _guard = EnvVarGuard::set("TOKENS_ANTIGRAVITY_MAX_RPC_BODY_BYTES", "1024");
        let cap = max_rpc_body_bytes();
    ...
    fn max_rpc_body_bytes_respects_env_var() {
        let _guard = EnvVarGuard::remove("TOKENS_ANTIGRAVITY_MAX_RPC_BODY_BYTES");
        assert_eq!(max_rpc_body_bytes(), DEFAULT_MAX_RPC_BODY_BYTES);
        assert_eq!(DEFAULT_MAX_RPC_BODY_BYTES, 128 * 1024 * 1024);

        let _guard = EnvVarGuard::set("TOKENS_ANTIGRAVITY_MAX_RPC_BODY_BYTES", "65536");
        assert_eq!(max_rpc_body_bytes(), 65536);

        // Non-positive or unparseable values fall back to the default.
        let _guard = EnvVarGuard::set("TOKENS_ANTIGRAVITY_MAX_RPC_BODY_BYTES", "0");
        assert_eq!(max_rpc_body_bytes(), DEFAULT_MAX_RPC_BODY_BYTES);

        let _guard = EnvVarGuard::set("TOKENS_ANTIGRAVITY_MAX_RPC_BODY_BYTES", "invalid");
        assert_eq!(max_rpc_body_bytes(), DEFAULT_MAX_RPC_BODY_BYTES);

        // usize::MAX cannot be safely incremented by readers and must fall back.
        let _guard = EnvVarGuard::set(
            "TOKENS_ANTIGRAVITY_MAX_RPC_BODY_BYTES",
            &usize::MAX.to_string(),
        );
        assert_eq!(max_rpc_body_bytes(), DEFAULT_MAX_RPC_BODY_BYTES);
    }
    ...
    fn trajectory_enrichment_failure_keeps_cached_timestamps_and_admits_new_usage() {
        ...
        let temp_dir = tempfile::tempdir().unwrap();
        let _env = TestEnvGuard::redirect_to(temp_dir.path());
        let _cap = EnvVarGuard::set("TOKENS_ANTIGRAVITY_MAX_RPC_BODY_BYTES", "1024");
    ...
    fn existing_sync_lock_error_names_the_exact_stale_lock_path() {
        ...
        assert!(err.contains("Confirm no tokens Antigravity sync is running"));
    ...
    fn sync_lock_guard_preserves_a_live_legacy_pid_lock() {
        ...
        assert!(
            err.to_string()
                .contains("Another tokens Antigravity sync may be in progress"),
    ...
    fn purge_cache_refuses_while_sync_holds_the_parent_lock() {
        ...
        assert!(
            err.to_string()
                .contains("Another tokens Antigravity cache operation is in progress"),
