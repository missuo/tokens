...
//! similar dashboards ingest). The importer normalizes either into tokens's
//! native [`GraphResult`], which the CLI then writes out as standard tokens
//! JSON (identical in shape to `tokens graph`) for review, archival, or a
...
struct CcusageAgentBreakdown {
    ...
    /// registered tokens client, so this needs no inference — only the same
...
/// Build a tokens [`TokenBreakdown`] from one ccusage row or model breakdown.
///
...
/// ccusage's documented examples). tokens's buckets are additive and
...
/// Parse a `ccusage daily --json` export into normalized tokens data.
///
...
pub fn parse_ccusage_export(json: &str) -> Result<ImportOutcome> {
    ...
    for row in &export.daily {
        ...
        // mapped through twice. Compare it against what tokens will report.
...
/// Convert one ccusage model breakdown into a tokens row.
///
...
/// usage, so the only work left is mapping ccusage's spelling onto tokens's
/// client id and flagging an id tokens does not register. Every id in
...
mod tests {
    ...
    fn ccusage_day_total_matches_the_export_declared_total() {
        ...
        // term of its own, while tokens's buckets are additive. Mapping
