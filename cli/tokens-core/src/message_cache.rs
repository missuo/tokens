...
/// concurrent tokens winning the race all just leave the file alone. The lock
/// file sharing its stem is still live and is never touched here.
///
/// The `legacy_*` probes return `None` under `TOKENS_CONFIG_DIR`, so an
...
impl CachedPath {
    ...
    /// file two cache keys.
    ///
    /// That is not only a test artifact. `tokens --home C:/Users/me` and a
...
// Fork (missuo/tokens) cache lineage.
//
// This fork shipped its own format 5 before it resynced with upstream: the
// upstream version-3 entry layout minus `UnifiedMessage::agent_run_id`, written
// by every release up to 27.0.5. No build of this fork ever wrote upstream's
// formats 4, 5 or 6, so on this fork's disk a 5 is always fork-5 and must never
// reach `LegacyCachedSourceEntryV5`: that layout expects a
// `model_attribution_conflicted` byte in every message, and bincode is not
// self-describing, so decoding fork-5 through it would shift every field that
// follows instead of failing cleanly. A 4 or a 6 cannot have been written here
// and is rebuilt rather than trusted.
const FORK_CACHE_FORMAT_VERSION_V5: u32 = LEGACY_CACHE_FORMAT_VERSION_V5;

/// Exact fork-5 message layout: today's [`UnifiedMessage`] without
/// `model_attribution_conflicted`, field order preserved.
#[derive(Debug, Serialize, Deserialize)]
struct ForkUnifiedMessageV5 {
    client: String,
    model_id: String,
    provider_id: String,
    session_id: String,
...
impl From<ForkUnifiedMessageV5> for UnifiedMessage {
    fn from(message: ForkUnifiedMessageV5) -> Self {
        Self {
...
#[derive(Debug, Serialize, Deserialize)]
struct ForkCachedSourceEntryV5 {
    parser_namespace: String,
    parser_version: u32,
    path: CachedPath,
    fingerprint: SourceFingerprint,
    messages: Vec<ForkUnifiedMessageV5>,
    fallback_timestamp_indices: Vec<usize>,
    codex_incremental: Option<CodexIncrementalCache>,
}

impl From<ForkCachedSourceEntryV5> for CachedSourceEntry {
    fn from(entry: ForkCachedSourceEntryV5) -> Self {
        Self {
...
impl CachedSourceEntry {
    ...
    ///
    /// Two processes can scan at once — a running TUI and a `tokens submit`,
...
/// whole scan made a `tokens` run cost the size of the entire cache plus the
...
fn read_shard_with_limit(
    ...
    // Fork lineage first; see `FORK_CACHE_FORMAT_VERSION_V5`.
    if envelope.format_version == FORK_CACHE_FORMAT_VERSION_V5 {
    ...
    if envelope.format_version == LEGACY_CACHE_FORMAT_VERSION_V4
        || envelope.format_version == LEGACY_CACHE_FORMAT_VERSION_V6
    {
        return ShardReadStatus::Stale;
    }
    if envelope.format_version == LEGACY_CACHE_FORMAT_VERSION_V4 {
...
mod tests {
    ...
    /// four keys here `TOKENS_CONFIG_DIR` is consulted first on every
    ...
    /// The checked-in DSH fixtures, under `crates/tokens-core/tests/fixtures`.
    ...
    /// Runs the production DSH lane the way `tokens --json` does: one cached
...
"#,
    ...
    #[test]
    #[serial_test::serial]
    fn test_fork_v5_shard_migrates_every_message_field() {
        let temp_home = TempDir::new().unwrap();
        let _cache_env = sandbox_cache_env(temp_home.path());
        let source = write_temp_file(b"{}\n");
        ...
        // Written in this fork's own format 5, which every release up to
        // 27.0.5 produced: no `model_attribution_conflicted` in the messages.
        let expected = entry.messages.clone();
        let legacy_entry = ForkCachedSourceEntryV5 {
            parser_namespace: entry.parser_namespace,
            parser_version: entry.parser_version,
            path: entry.path,
            fingerprint: entry.fingerprint,
        ...
        let envelope = CachedShardEnvelope {
            format_version: FORK_CACHE_FORMAT_VERSION_V5,
            parser_namespace: identity.namespace.to_string(),
            parser_version: identity.parser_version,
        ...
        match read_shard(&legacy_path, identity) {
            ShardReadStatus::Migrated(entries) => {
                ...
                assert!(entries[0].opencode_incremental.is_none());
            }
            _ => panic!("fork-5 shard must migrate"),
        }
    ...
    #[test]
    #[serial_test::serial]
    fn test_v4_shard_is_foreign_to_this_fork_and_rebuilds() {
        // Upstream's format 4 was never written by a build of this fork, so a
        // shard claiming it is rebuilt rather than decoded.
        let temp_home = TempDir::new().unwrap();
        let _cache_env = sandbox_cache_env(temp_home.path());
        let source = write_temp_file(b"{}\n");
    ...
    fn a_legacy_format_shard_whose_payload_will_not_decode_is_invalid() {
        ...
        ensure_cache_dir(path.parent().unwrap()).unwrap();
        // Fork-5 is the only legacy layout this fork still decodes.
        let envelope = CachedShardEnvelope {
            format_version: FORK_CACHE_FORMAT_VERSION_V5,
            parser_namespace: identity.namespace.to_string(),
            parser_version: identity.parser_version,
