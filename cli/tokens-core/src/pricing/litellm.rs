...
impl ModelPricing {
    ...
    /// tokens reports both at $0.00 and cannot do better from this input
...
/// `~/.config/tokens/cache/pricing-litellm.json`. Any fixture-server test
...
/// `TOKENS_CONFIG_DIR`: the override is process-global, so it only protects
...
mod tests {
    ...
    // test without any tokens defect.
    #[tokio::test]
    ...
    /// `~/.config/tokens/cache/pricing-litellm.json`, evicting the genuine
    ...
    ///
    /// The assertion redirects `TOKENS_CONFIG_DIR` at a `TempDir` instead of
    ...
    ///
    /// `#[serial]` is load-bearing for the same reason. `TOKENS_CONFIG_DIR`
    ...
    async fn a_fetch_with_caching_disabled_writes_no_cache_file() {
        ...
        let mut env = EnvGuard::capture(&["TOKENS_CONFIG_DIR"]);
        env.set("TOKENS_CONFIG_DIR", temp_config.path());
