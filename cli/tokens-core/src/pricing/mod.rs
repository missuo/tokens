...
impl PricingService {
    ...
    // Tokens's own fuzzy election of a neighbouring `gpt-5.1-codex-mini` row,
    ...
    fn degrade_source(
        ...
        match result {
            ...
            Err(error) => {
                let cached = cached();
                eprintln!(
                    "[tokens] Warning: {} pricing fetch failed ({}); {}",
                    label,
                    error,
                    if cached.is_some() {
...
mod tests {
    ...
    fn assert_retired_anthropic_price(
        ...
        assert_eq!(resolved.source, "Tokens Archive", "model: {model}");
    ...
    fn zhipu_glm_rates_resolve_first_party_over_reseller_row() {
        ...
        for model in ["glm-5.2", "glm-5.3"] {
            ...
            for (provider, test_model, expected_key) in [
                ...
                assert_eq!(resolved.source, "Tokens Archive", "model: {test_model}");
                assert_eq!(
    ...
    fn xiaomi_mimo_rate_resolves_first_party_over_marketplace_row() {
        ...
        for (provider, model_id, expected_key) in [
            ...
            assert_eq!(resolved.source, "Tokens Archive");
    ...
    fn reseller_rooted_id_gets_an_archive_estimate_but_not_submission_safety() {
        ...
        assert_eq!(resold.source, "Tokens Archive");
        assert_eq!(
        ...
        assert_eq!(first_party.source, "Tokens Archive");
    ...
    fn retired_anthropic_price_reaches_vertex_only_as_an_estimate() {
        ...
        for provider in ["vertex", "vertex_ai", "Vertex"] {
            ...
            assert_eq!(resolved.source, "Tokens Archive", "provider: {provider}");
            assert_eq!(
    ...
    fn codex_auto_review_is_never_served_from_the_archive() {
        ...
        assert!(
            resolved
                .as_ref()
                .is_none_or(|result| result.source != "Tokens Archive"),
    ...
    // fetch_inner, so `tokens submit` died with "error decoding response
    ...
    fn suffix_eroded_sku_keeps_its_live_dataset_price() {
        ...
        let base = service
            ...
            .expect("mimo-v2.5 must resolve first-party");
        assert_eq!(base.source, "Tokens Archive");
    }
}
