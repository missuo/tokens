//! Cross-platform resolution for tokens's user config and cache dirs.
//!
...
/// `test_ensure_config_dir` was creating `%USERPROFILE%\.config\tokens` on
...
///   tokens from a Unix-shell emulator to `C:\home\user`.
...
pub fn legacy_dot_cache_tokens_dir() -> Option<PathBuf> {
    ...
    home_dir().map(|h| h.join(".cache").join("tokens"))
}

/// RAII restore of process-global environment variables, shared by the tests
/// in this crate that redirect `HOME` or `TOKENS_CONFIG_DIR`.
///
...
mod tests {
    ...
    fn guard() -> EnvGuard {
        EnvGuard::capture(&["TOKENS_CONFIG_DIR", "HOME", "XDG_CONFIG_HOME"])
    }
    ...
    fn env_guard_restores_even_when_the_test_body_panics() {
        const SENTINEL: &str = "TOKENS_ENV_GUARD_PANIC_PROBE";
    ...
    fn home_dir_follows_an_explicit_native_home_on_every_platform() {
        ...
        let redirect = std::env::temp_dir().join("tokens-core-home-dir-probe");
    ...
    fn env_override_is_returned_verbatim() {
        let mut env = guard();
        env.set("TOKENS_CONFIG_DIR", "/tmp/tokens-custom");
        assert_eq!(get_config_dir(), PathBuf::from("/tmp/tokens-custom"));
    }
    ...
    fn unix_default_is_dot_config_tokens_under_home() {
        let mut env = guard();
        env.remove("TOKENS_CONFIG_DIR");
        env.remove("XDG_CONFIG_HOME");
        env.set("HOME", "/tmp/tokens-core-paths-home");
        assert_eq!(
            get_config_dir(),
            PathBuf::from("/tmp/tokens-core-paths-home/.config/tokens"),
        );
    }
    ...
    fn linux_honors_xdg_config_home_when_set() {
        let mut env = guard();
        env.remove("TOKENS_CONFIG_DIR");
        env.set("XDG_CONFIG_HOME", "/tmp/tokens-core-paths-xdg");
        assert_eq!(
            get_config_dir(),
            PathBuf::from("/tmp/tokens-core-paths-xdg/tokens"),
        );
    }
    ...
    fn cache_dir_is_cache_subdir_of_config_dir() {
        let mut env = guard();
        env.set("TOKENS_CONFIG_DIR", "/tmp/tokens-cache-test");
        assert_eq!(
            get_cache_dir(),
            PathBuf::from("/tmp/tokens-cache-test/cache")
        );
    }
    ...
    fn legacy_helpers_return_none_when_overridden() {
        let mut env = guard();
        env.set("TOKENS_CONFIG_DIR", "/tmp/tokens-override");
    ...
    fn get_config_dir_treats_empty_override_as_unset() {
        ...
        assert!(
            resolved.is_absolute() || resolved == Path::new(".tokens"),
