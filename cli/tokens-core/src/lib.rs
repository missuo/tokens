...
fn parse_all_messages_streaming<S: MessageSink>(
    ...
    // synchronized by `tokens hindsight sync`. Request IDs provide a stable
...
fn aggregate_model_usage_entries_with_rollup(
    ...
    // than paying it on `tokens --light`, `monthly`, and every TUI refresh.
...
pub fn parse_local_clients(options: LocalParseOptions) -> Result<ParsedMessages, String> {
    ...
    // same filter here, local `tokens` output double-counts a forked
    ...
    // needs its own rebucket — `tokens report` and `tokens wrapped` read
    // day keys from here.
...
"#,
    ...
    /// `%APPDATA%\tokens\cache` and loaded back the shards its neighbours had
    ...
    ///
    /// `TOKENS_CONFIG_DIR` is the override `paths.rs` documents for exactly
    ...
    /// `TOKENS_CONFIG_DIR` is consulted first on *every* platform, so a leaked
    ...
    fn redirect_cache_home(home: &std::path::Path) -> crate::paths::test_env::EnvGuard {
        let mut env = crate::paths::test_env::EnvGuard::capture(&["HOME", "TOKENS_CONFIG_DIR"]);
    ...
    fn point_cache_home(env: &mut crate::paths::test_env::EnvGuard, home: &std::path::Path) {
        env.set("HOME", home);
        env.set("TOKENS_CONFIG_DIR", home.join(".config").join("tokens"));
    }
    ...
    fn get_home_dir_string_prefers_the_explicit_option() {
        ...
        env.set("HOME", "/tmp/tokens-env-home");
        assert_eq!(
            get_home_dir_string(&Some("/tmp/tokens-explicit-home".to_string())),
            Ok("/tmp/tokens-explicit-home".to_string())
        );
    }
    ...
    fn workspace_labeler_decodes_each_key_once() {
        ...
        let key = "-nonexistent-tokens-decode-probe";
...
"2026-03-04T12:00:00.000Z","Included","Composer 1.5","No","1200","1000","5000","2000","8000","0""#;
    ...
    /// good — and `tokens submit` feeds the leaderboard from the same
...
{"ts":"2026-07-31T00:00:04Z","pid":19,"sid":"child","msg":"shell.turn.inference_done","ctx":{"loop_index":1,"prompt_tokens":100,"cached_prompt_tokens":60,"completion_tokens":25,"reasoning_tokens":5}}"#,
    ...
    fn test_parse_local_clients_includes_micode_and_fx() {
        ...
        // built on it saw nothing: `tokens clients` reported a zero message
        // count, and the `tokens report` session wiki and `tokens wrapped`
    ...
    fn test_parse_all_messages_with_pricing_includes_opencodereview() {
        ...
        // parse_local_clients has always parsed it, so `tokens report`
...
{"type":"message","id":"msg_absent","message":{"role":"assistant","model":"gpt-4o","provider":"openai","timestamp":1733011201000,"usage":{"input":10,"output":5}}}"#,
    ...
    fn test_submit_default_graph_includes_antigravity_cache_rows() {
        ...
        // root is `~/.config/tokens` on Unix and
        // `%HOME%\AppData\Roaming\tokens` on Windows, so the Unix spelling
