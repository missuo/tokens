...
also exercised with a live model request, and that print-mode run left an
empty transcript, so the live request did not verify persistence. The fixtures
are read by `tests/commandcode_released_writers.rs`.
...
rewriting them. Unknown version markers and malformed v3 records are not reinterpreted as
legacy messages.
