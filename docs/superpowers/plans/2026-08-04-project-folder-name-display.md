...
**Architecture:** Claude session parsing keeps the path-derived `workspace_key` as identity while deriving `workspace_label` from the latest usable JSONL `cwd` only when that key is non-nil. Duplicate merges preserve the final valid label for timestamp-based project aggregation. Aggregation promotes attributed labels into report `displayName`, while nil project keys are forced to `Unattributed` so cwd data cannot leak into JSON. Claude cwd extraction and Swift presentation split only on `/`; literal backslashes remain part of non-path names. The menu bar normalizes attributed project labels in this order: usable `displayName`, usable `projectKey`, then `Unattributed`. Report schema fields remain unchanged.
...
- Treat empty, whitespace-only, root-only, and all-`/` names as unusable.
- A nil project key is unattributed, never accepts or propagates a cwd-derived label, always reports/renders `Unattributed`, and never exposes display-name or path data.
- Treat only `/` as a path separator for this feature; preserve literal `\` characters in cwd components and non-path display names.
...
- `cli/tokens-cli/src/commands/usage_report.rs` — force nil project keys to report `Unattributed` even if an upstream label is present.
- `Sources/TokensMenuBarCore/Models.swift` — normalize attributed project names by trying usable `displayName`, then usable `projectKey`, then `Unattributed`, splitting only on `/`.
...
5. A whitespace-only final cwd component does not replace the previous valid/path-derived label.
6. Literal backslashes remain part of the final cwd component because only `/` is a separator.
7. An assistant duplicate carrying a later valid cwd refreshes the deduplicated message label.
...
2. Consider cwd labels only when the path-derived `workspace_key` is non-nil.
3. Strip trailing `/`, select the final `/`-separated component, trim that component, and reject empty/whitespace/root-only results.
4. Preserve literal backslashes; do not run cwd through cross-platform separator normalization.
5. Keep the path-derived `workspace_key` unchanged.
6. Ignore unusable cwd values so the previous valid or path-derived label remains available.
...
```bash
cargo test --manifest-path cli/Cargo.toml -p tokens-cli commands::usage_report::tests -- --nocapture
```

Expected: the bare-transcript parser-to-report regression and the defensive nil-key report regression pass without changing schema, totals, sorting, or pagination.
...
Create `ProjectUsageTests` covering these thirteen cases:
...
9. Literal backslashes in a non-path `displayName` remain unchanged.
10. Path-shaped display names split only on `/`, preserving backslashes inside the final component.
11. Nil `projectKey` always returns `Unattributed`, even if `displayName` contains a path.
12. Empty names return `Unattributed`.
13. Whitespace/all-`/` `displayName` and `projectKey` values return `Unattributed`.
...
2. Use one private helper to trim whitespace, split only on `/`, discard empty/all-`/` inputs, preserve literal backslashes, and return the final usable component.
...
Expected: all thirteen `ProjectUsageTests` pass.
...
cargo test --manifest-path cli/Cargo.toml -p tokens-cli commands::usage_report::tests
swift test
```
...
6. Nil keys always remain `Unattributed` in parsed messages, aggregation, report JSON, and Swift presentation, with no cwd-derived label leakage.
7. Literal backslashes remain visible; only `/` separates path components.
8. Nested-model accessibility uses the same folder name.
...
git diff origin/main...HEAD -- \
  ...
  cli/tokens-cli/src/commands/usage_report.rs \
...
Stage only the eight files listed above and create one focused conventional commit ending with:
