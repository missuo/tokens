...
Tokens can surface your Sakana (Fugu) subscription quota windows in the
`tokens usage` command and the TUI Usage tab.
...
> `GET`, so tokens fetches that HTML with your session cookie and parses it.
...
- **Quota windows, not dollars.** Sakana subscription billing is a flat monthly
  fee (e.g. `$20/mo`), so there is no per-request spend to report. Tokens shows
...
- Or a file (raw cookie string, trimmed):
  ...
  ( umask 077 && printf '%s' '__Secure-authjs.session-token.0=...; __Secure-authjs.session-token.1=...' \
      > ~/.config/tokens/sakana-session )
  # Alternative: install -m 600 /dev/null ~/.config/tokens/sakana-session first.
  ```

  The file lives in the Tokens config dir, which honors `TOKENS_CONFIG_DIR`
  (and `$XDG_CONFIG_HOME/tokens` on Linux); `~/.config/tokens` is just the
  common default.
...
Tokens detects this and reports a clear error asking you to refresh
...
keep `~/.config/tokens/sakana-session` at mode `600`, and don't commit it or
