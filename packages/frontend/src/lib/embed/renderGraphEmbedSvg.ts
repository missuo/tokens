...
export function renderGraphEmbedSvg(
  ...
  const H = Math.ceil(58 + graph.height + 32) + (options.today ? 26 : 0);
  const stats = [
...
<svg data-template="graph" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Tokens contribution graph for @${escapeXml(data.user.username)}">
  ${cardTextStyle()}
  ...
  ${fittedText({ text: stat.value, x, y: 42, maxWidth, fill: stat.color, fontSize: 14, minFontSize: 7, fontWeight: 600 })}`;
    ...
    .join("\n  ")}
  ${graph.svg}
  ${options.today ? todayMetricLine({ today: options.today, palette, x: PAD, right, y: H - 34, tokensCompact: (options.tokensFormat ?? "compact") === "compact", costCompact: (options.costFormat ?? "compact") === "compact" }) : ""}
  ${cardFooter({
