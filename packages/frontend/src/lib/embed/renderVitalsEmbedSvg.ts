...
export function renderVitalsEmbedSvg(
  ...
  add(`<svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Tokens activity rings for @${escapeXml(data.user.username)}">`);
  add(`  <defs>`);
  ...
  add(`  <text x="${W - PAD}" y="${H - 16}" fill="${palette.muted}" font-size="11" text-anchor="end" font-family="${FIGTREE_FONT_STACK}">tokens.ci/u/${escapeXml(data.user.username)}</text>`);
  add(`</svg>`);
