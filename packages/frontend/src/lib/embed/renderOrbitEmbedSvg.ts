...
export function renderOrbitEmbedSvg(
  ...
  const graph = contributions
    ...
    : null;
  const height = (graph ? 384 : 248) + (options.today ? 26 : 0);
  const statRows = [
    {
      label: "Tokens",
      value: formatNumber(
...
<svg data-template="orbit" width="${W}" height="${height}" viewBox="0 0 ${W} ${height}" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Tokens leaderboard standing for @${escapeXml(data.user.username)}">
  ${cardTextStyle()}
  ...
  ${options.today ? todayMetricLine({ today: options.today, palette, x: PAD, right, y: height - 34, tokensCompact: (options.tokensFormat ?? "compact") === "compact", costCompact: (options.costFormat ?? "compact") === "compact" }) : ""}
  ${cardFooter({
