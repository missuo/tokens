...
export function renderOrbitEmbedSvg(
  ...
  add(`<svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Tokens rank gauge for @${escapeXml(data.user.username)}">`);
  add(`  <defs>`);
