...
export function renderBlueprintEmbedSvg(
  ...
  const graph = contributions
    ...
    : null;
  const height = (graph ? 367 : 232) + (options.today ? 26 : 0);

  return `<?xml version="1.0" encoding="UTF-8"?>
<svg data-template="blueprint" width="${W}" height="${height}" viewBox="0 0 ${W} ${height}" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Tokens usage data sheet for @${escapeXml(data.user.username)}">
  ${cardTextStyle()}
  ...
  ${options.today ? todayMetricLine({ today: options.today, palette, x: PAD, right, y: height - 34, tokensCompact, costCompact }) : ""}
  ${cardFooter({
