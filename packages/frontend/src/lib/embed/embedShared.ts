...
export const EMBED_TODAY_CAPABLE = EMBED_TEMPLATES.filter(
...
/** Three ascending bars used as the Tokens brand mark. */
...
export function cardHeader(options: CardHeaderOptions): string {
  ...
  return [
    ...
    displayName
      ...
      : "",
    `<text x="${right}" y="${y}" fill="${palette.muted}" font-size="11" font-weight="600" text-anchor="end" font-family="${FIGTREE_FONT_STACK}">Tokens</text>`,
  ]
    .filter(Boolean)
    .join("\n  ");
}
