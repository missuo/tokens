...
export function renderMinimalEmbedSvg(
  ...
  const graph = contributions
    ...
    : null;
  const baseHeight = graph ? Math.ceil(graphY + graph.height + 32) : 162;
  const height = baseHeight + (options.today ? 26 : 0);

  return `<?xml version="1.0" encoding="UTF-8"?>
<svg data-template="minimal" width="${W}" height="${height}" viewBox="0 0 ${W} ${height}" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Tokens stats for @${escapeXml(data.user.username)}">
  ${cardTextStyle()}
  ...
  ${options.today ? todayMetricLine({ today: options.today, palette, x: PAD, right, y: height - 34, tokensCompact: tokensFormat === "compact", costCompact: costFormat === "compact" }) : ""}
  ${cardFooter({
