...
describe("renderTerminalEmbedSvg", () => {
  it("uses monospace result grammar without inventing a profile command", () => {
    ...
    expect(svg).toContain("<svg");
    expect(svg).toContain("@octocat");
    expect(svg).not.toContain("tokens profile");
...
const ariaLabels: Record<keyof typeof allTemplates, string> = {
  classic: "Tokens profile stats for @octocat",
  minimal: "Tokens stats for @octocat",
  terminal: "Tokens stats for @octocat",
  graph: "Tokens contribution graph for @octocat",
  orbit: "Tokens leaderboard standing for @octocat",
  vitals: "Tokens usage signals for @octocat",
  blueprint: "Tokens usage data sheet for @octocat",
  receipt: "Tokens compact ledger for @octocat",
};
