...
[![npm Version](https://img.shields.io/npm/v/tokens-cli?color=0073FF&labelColor=black&style=flat-square&logo=npm)](https://www.npmjs.com/package/tokens-cli)
...
[![License](https://img.shields.io/badge/license-MIT-white?labelColor=black&style=flat-square)](https://github.com/missuo/tokens/blob/master/LICENSE)
...
> **Run [`bunx tokens@latest submit`](#social) to submit your usage data to the leaderboard and create your public profile!**
...
| <img width="48px" src="https://github.com/warpdotdev.png" alt="Warp" /> | [Warp](https://www.warp.dev/) / Oz | Cached via `tokens warp sync` to `~/.config/tokens/warp-cache/usage.json` (aggregate requests and spend only; no token transcripts) | ✅ Yes |
...
[![Tokens](./.github/assets/hero.png)](https://tokens.ci)
...
deno x npm:tokens@latest

# Light mode (table rendering only)
npx tokens@latest --light
```

That's it! This gives you the full interactive TUI experience with zero setup.

> **Package Structure**: `tokens` is an alias package (like [`swc`](https://www.npmjs.com/package/swc)) that installs `@tokens/cli`. Both install the same CLI with the native Rust core (`@tokens/core`) included.
...
> **Note**: `bun run cli` is for local development. When installed via `bunx tokens`, the command runs directly. The Usage section below shows the installed binary commands.
...
> **Note**: Native binaries are pre-built and included when you install via `bunx tokens@latest`. Building from source is only needed for local development.
...
tokens models    # Models tab
tokens monthly   # Daily view (shows daily breakdown)
tokens hourly    # Hourly tab
...
tokens --json                    # Default models view as JSON
tokens models --json             # Models breakdown as JSON
tokens monthly --json            # Monthly breakdown as JSON
tokens models --json > report.json   # Save to file
```
...
- **Settings Persistence**: Preferences saved to `~/.config/tokens/settings.json` (see [Configuration](#configuration))
...
`tokens models --json --group-by session,model` emits one entry per `(session_id, model)`. Each entry includes a top-level `sessionId` field so downstream tools (e.g. multi-agent IDEs) can join cost data back to a specific agent-CLI session:
...
# Cursor IDE uses Tokens's API cache; run login + sync --json first
...
> **Breaking change (v4.0.0):** The per-client boolean flags (`--opencode`, `--claude`, `--codex`, etc.) have been removed and now error. Use the canonical `--client`/`-c` flag instead — e.g. `tokens --client opencode,claude`.

### Date Filtering

Date filters work across all commands that generate reports (`tokens`, `tokens models`, `tokens monthly`, `tokens graph`):

```bash
# Quick date shortcuts
tokens --today              # Today only
tokens --yesterday          # Yesterday only
tokens --week               # Last 7 days
tokens --month              # Current calendar month
...
1. **Custom Pricing Overrides** - Exact user-defined entries from `~/.config/tokens/custom-pricing.json`
...
Create `custom-pricing.json` in Tokens's config directory (`~/.config/tokens/custom-pricing.json` on macOS/Linux by default; the same directory resolved by `TOKSCALE_CONFIG_DIR` when set) to override prices for model IDs that upstream pricing databases do not yet cover correctly.

```json
{
  "$schema": "https://tokens.ci/custom-pricing.schema.json",
  "models": {
...
```bash
# Login to Tokens (opens browser for GitHub auth)
tokens login

# Save an existing Tokens API token without browser auth
tokens login --token tt_xxx
...
# Precedence: TOKSCALE_API_TOKEN env > saved credentials file (~/.config/tokens/credentials.json).
# When the env var is set, the saved file is ignored for that invocation.
TOKSCALE_API_TOKEN=tt_xxx tokens submit

# Revoke a token: visit Settings > API Tokens on the leaderboard site
# (https://tokens.ci/settings) and click "Revoke" on the token row.
...
Cursor IDE support uses Cursor's web API export, cached by Tokens at `~/.config/tokens/cursor-cache/usage*.csv`. Tokens does not parse local Cursor Agent CLI state under `~/.cursor`.
...
3. Run `tokens cursor login --name work` and paste the token.
4. Run `tokens cursor sync --json` to populate `~/.config/tokens/cursor-cache/usage.csv`.
5. Run `tokens --client cursor` or any report command.

Treat the session token like a password. It is stored locally in `~/.config/tokens/cursor-credentials.json`.
...
By default, Tokens aggregates usage across all saved Cursor accounts by reading `cursor-cache/usage*.csv`. The active account syncs to `usage.csv`; additional accounts sync to `usage.<account>.csv`.

When you log out, Tokens moves cached usage to `cursor-cache/archive/` so it is no longer aggregated. Use `--purge-cache` to delete cached usage instead.

### Antigravity Commands

Antigravity sync currently works on macOS and Linux only. The Antigravity-enabled editor must be running and its local language server available; tokens reads usage from that local language server and caches normalized artifacts locally.

```bash
# Check whether tokens can see running Antigravity language servers
tokens antigravity status

# Sync usage from local Antigravity language servers into tokens's cache
...
**Cache location**: `~/.config/tokens/antigravity-cache/`

**How it works**: `tokens antigravity sync` discovers local Antigravity session candidates, fetches confirmed usage data from the local language server RPC, and stores normalized JSONL artifacts for tokens-core to parse later. Run sync before reports if you want the freshest Antigravity data.

### Trae Commands

Trae ([ByteDance's AI IDE](https://www.trae.ai/)) ships in two international product lines — Trae IDE and Trae Solo. They share the same account-level usage data (same backend, same JWT), so tokens reports them as a single `trae` client. You can install either or both desktop apps; tokens auto-discovers credentials from whichever is present.
...
`tokens trae sync` calls the official `query_user_usage_group_by_session` API exactly once per run (regardless of how many desktop apps are installed) and persists the raw JSON to a local cache.
...
**Cache location**: `~/.config/tokens/trae-cache/`

**How it works**: tokens either decrypts the desktop client's `iCubeAuthInfo://*` blob (`globalStorage/storage.json`) to recover a JWT, or accepts one pasted via `--manual`. It then calls `POST /trae/api/v1/pay/query_user_usage_group_by_session` paginated and stores the raw JSON. Run sync before reports if you want the freshest Trae data.

> **Note on pricing**: Trae cost figures are **vendor-reported** — tokens surfaces the `dollar_float` value returned by Trae's own API rather than recomputing cost from token counts through tokens's pricing engine. Numbers will match what you see on `trae.ai/account-setting#usage`, not what tokens would otherwise calculate for the same usage.
...
Warp/Oz does not expose local token transcripts. Tokens only syncs the aggregate request and spend counters returned by Warp's GraphQL API, then reports them as `warp` / `aggregate-requests` rows with zero token buckets.
...
# Sync aggregate requests and spend into tokens's local cache
tokens warp sync

# Remove saved credentials; add --purge-cache to delete synced usage too
tokens warp logout --purge-cache
```

**Cache location**: `~/.config/tokens/warp-cache/usage.json`

**How it works**: `tokens warp sync` calls Warp's authenticated GraphQL API for account and workspace aggregate counters. Tokens preserves request counts as message counts and vendor-reported spend as cost, but it never converts requests into synthetic tokens. Warp is excluded from default `submit` data because the public leaderboard accepts token-attributed usage, not aggregate request counters.
...
tokens report --week --summarizer claude

# Use Codex, Gemini, or Kiro
tokens report --summarizer codex
tokens report --summarizer gemini
tokens report --summarizer kiro

# Skip LLM summarization (show raw data only)
tokens report --no-summarize

# Re-summarize from scratch (resets cached summaries in range)
tokens report --week --rebuild

# Output as JSON
tokens report --week --json

# Filter by workspace or client
tokens report --workspace my-project --client opencode
```
...
1. Sessions are scanned and inserted into a local SQLite wiki database (`wiki.db` in your platform config dir — e.g. `~/.config/tokens/` on Linux, `~/Library/Application Support/tokens/` on macOS)
...
3. A second LLM pass groups all titled sessions into 3–8 high-level task clusters (e.g. "Kiro Auth", "Tokens Report", "System Config")
...
```
  ...
  ───────────────────────────────────────────────────────────────────────
  Tokens Development                          19      4.2B    $22.66
...
Tokens can fetch and display your real-time subscription quota across AI providers. This shows how much of your plan you've used and when limits reset.
...
> **Note**: Subscription quotas and balances are **vendor-reported** — tokens calls each provider's own quota endpoint and surfaces the response verbatim. Numbers reflect what the provider reports (which is also what shows up in their official dashboards) and are not independently verified against tokens's own usage tracking.
...
| **Codex** (OpenAI) | OAuth (`~/.config/codex/auth.json`, `~/.codex/auth.json`, or saved Tokens accounts) | Session, Weekly quotas | Use `[Add Codex]` in the TUI Usage tab, run `codex` to log in, or import an existing auth with `tokens codex import --name work` |
...
Tokens can save multiple Codex OAuth accounts for subscription usage display. The TUI Usage tab groups saved accounts under one **Codex** section. The active account is marked with `*`; inactive accounts can be selected with `[Use]`; account removal uses `[Remove]` followed by `[Confirm]`.

To add an account without leaving the TUI, click `[Add Codex]` in the Usage tab. Tokens starts `codex login` with a temporary `CODEX_HOME`, displays the login output in the Usage tab, imports the resulting auth into Tokens's saved account store, and then refreshes usage. This keeps the login isolated and does not switch the current Codex auth; click `[Use]` on a saved account when you want Tokens to write that account into the real Codex auth file.

The CLI commands are still available for scripted or manual account management:

```bash
# Save the current Codex auth as a named Tokens account
tokens codex import --name work

# List saved Codex accounts
tokens codex accounts
tokens codex accounts --json

# Switch the active Codex account and write Codex auth.json
tokens codex switch work

# Stop tracking a saved Codex account (removes it from Tokens's store
# only — the codex CLI's own auth.json/login is never touched)
tokens codex remove personal

# Check subscription usage for the active or a named account
tokens codex status
tokens codex status --name personal --json
```

When saved Codex accounts exist, `tokens usage --json` includes structured account metadata for each Codex entry and the TUI displays those entries under one Codex group. Without saved accounts, Tokens falls back to the current Codex auth discovery path (`CODEX_HOME/auth.json`, `~/.config/codex/auth.json`, `~/.codex/auth.json`, then macOS Keychain).
...
Tokens stores settings in `~/.config/tokens/settings.json`:
...
| `light.writeCache` | boolean | `false` | When true, `tokens --light` overwrites the TUI cache atomically after rendering. CLI flags `--write-cache` / `--no-write-cache` override per-invocation. |
...
| `scanner.extraScanPaths` | object | `{}` | Additional per-client scan roots for sessions outside Tokens's default home-root locations |

Use `scanner.extraScanPaths` for persistent extra roots such as project-level `.codex` directories, imported Gemini/OpenClaw histories, or Hermes profile databases. Hermes entries may point at a profile directory containing `state.db` or directly at a `state.db` file. Tokens merges these paths with the default scan roots on every run and deduplicates overlapping roots by canonical path.

Use `defaultClients` to pin a personal default — for example, set it to `["opencode", "claude"]` if those are the only clients you use, and `tokens` (with no flags) will scope every report to them automatically. Pass `--client` on the command line to override for a single run.
...
To enable it, set `minutelyTabEnabled` to `true` in `~/.config/tokens/settings.json`:
...
The regenerable CLI/TUI/pricing/Wrapped caches now live under `~/.config/tokens/cache/` (or `${TOKSCALE_CONFIG_DIR}/cache/` when overridden). Integration sync artifacts remain in client-specific cache roots such as `~/.config/tokens/antigravity-cache/` and `~/.config/tokens/trae-cache/`:
...
It is safe to delete this directory. Tokens will recreate and repopulate it on demand.
...
| `TOKSCALE_API_TOKEN` | unset | Tokens personal API token for non-interactive `submit` and `delete-submitted-data` runs. Create one from Settings > API Tokens or save it locally with `tokens login --token tt_xxx`. |
| `TOKSCALE_EXTRA_DIRS` | unset | One-off extra session roots as `client:/abs/path,client:/abs/path` |
| `TOKSCALE_CONFIG_DIR` | unset | Overrides the config directory root (where `settings.json`, `star-cache.json`, `cache/`, `antigravity-cache/`, and `trae-cache/` live). Absolute path recommended; relative paths resolve against the process CWD. Useful for CI sandboxes or pinning a non-default location. When set, tokens will not fall back to the legacy macOS `~/Library/Application Support/tokens/` path. |
...
TOKSCALE_NATIVE_TIMEOUT_MS=600000 tokens graph --output data.json

# Example: one-off extra scan roots
TOKSCALE_EXTRA_DIRS='codex:/Users/me/workspace/project-a/.codex/sessions,gemini:/Users/me/imports/imac/gemini/tmp' tokens

# Example: submit from CI without an interactive browser login
TOKSCALE_API_TOKEN=tt_xxx tokens submit
```

> **Note**: For persistent extra roots, prefer `scanner.extraScanPaths` in `~/.config/tokens/settings.json`. `TOKSCALE_EXTRA_DIRS` is best for one-off overrides or CI/CD.

### Headless Mode

Tokens can aggregate token usage from **Codex CLI headless outputs** for automation, CI/CD pipelines, and batch processing.
...
**Storage location:** `~/.config/tokens/headless/`

On macOS, Tokens also scans `~/Library/Application Support/tokens/headless/` when `TOKSCALE_HEADLESS_DIR` is not set.

Tokens automatically scans this directory structure:
```
...
Tokens includes a social platform where you can share your usage data and compete with other developers.
...
You can embed your public Tokens stats directly in your GitHub profile README:
...
```md
![](https://tokens.ci/api/embed/<username>/svg?template=minimal&color=purple&graph=1)
![](https://tokens.ci/api/embed/<username>/svg?template=orbit&color=pink&rank=percent)
![](https://tokens.ci/api/embed/<username>/svg?template=terminal&color=green&theme=light)
![](https://tokens.ci/api/embed/<username>/svg?template=receipt&color=YlGnBu&graph=1)
```
...
1. **Login** - Run `tokens login` to authenticate via GitHub, or create an API token in Settings for CI/headless use
2. **Submit** - Run `tokens submit` to upload your usage data
...
| `bunx tokens@latest wrapped` | `bunx tokens@latest wrapped --clients` | `bunx tokens@latest wrapped --agents --disable-pinned` |
|:---:|:---:|:---:|
...
```bash
tokens --benchmark           # Show processing time with default view
tokens models --benchmark    # Benchmark models report
tokens monthly --benchmark   # Benchmark monthly report
tokens graph --benchmark     # Benchmark graph generation
```
...
Tokens fully supports Windows. The TUI and CLI work the same as on macOS/Linux.
...
| Antigravity | `~/.config/tokens/antigravity-cache/sessions/` | — | `tokens antigravity sync` is currently supported on macOS/Linux only |
...
| Trae | `~/.config/tokens/trae-cache/sessions/` | `%APPDATA%\tokens\trae-cache\sessions\` | Synced once via `tokens trae sync`; credentials are auto-discovered from any installed Trae IDE or Trae Solo desktop app |
| Warp/Oz | `~/.config/tokens/warp-cache/usage.json` | `%APPDATA%\tokens\warp-cache\usage.json` | Synced via `tokens warp sync`; aggregate requests and spend only, no token transcripts |
...
Tokens stores its configuration in:
- **TUI settings**: `%APPDATA%\tokens\settings.json` (platform default; override with `TOKSCALE_CONFIG_DIR`)
- **Cache**: `%APPDATA%\tokens\cache\` (consolidated cache root)
- **Legacy cache paths**: `%USERPROFILE%\.cache\tokens\` and `%LOCALAPPDATA%\tokens\cache\` equivalents from older releases may still exist until regenerated data is written to the new path
- **Cursor credentials**: `%USERPROFILE%\.config\tokens\cursor-credentials.json`
- **Trae credentials and synced usage**: `%APPDATA%\tokens\trae-cache\`
- **Tokens account credentials**: `%USERPROFILE%\.config\tokens\credentials.json`
...
OpenCode 1.2+ stores sessions in SQLite. Tokens reads from SQLite first and falls back to legacy JSON files for older versions.

OpenCode picks the db filename from the release channel the binary was built against: the `latest` and `beta` channels use `opencode.db`, while other channels use `opencode-<channel>.db` (e.g. `opencode-stable.db`, `opencode-nightly.db`). Tokens scans all of them, so users running multiple channels side by side get a unified view.

If you launched opencode with `OPENCODE_DB` pointing at a file outside `~/.local/share/opencode`, add the absolute path to `~/.config/tokens/settings.json` so tokens can find it on every run:
...
If you keep sessions outside Tokens's default home-root locations, you can also persist extra scan roots per client:
...
This is useful for project-level `.codex` directories, imported histories, and Hermes profile databases outside the default `$HERMES_HOME/state.db` or `~/.hermes/state.db` location. Tokens still scans its default roots, then merges `scanner.extraScanPaths` and `TOKSCALE_EXTRA_DIRS` on top with canonical-path deduplication. It does not auto-discover your whole workspace.
...
Tokens's `claude` client is Claude Code token accounting, not Claude Desktop chat accounting. Claude Desktop stores app data under locations such as `~/Library/Application Support/Claude`, but Anthropic does not document a stable local per-message token ledger for consumer desktop chat or chat-history exports. Run `tokens clients` to see a diagnostic when Claude Desktop data is present but only Claude Code JSONL roots are scannable. `tokens usage` can show best-effort Claude subscription quota bars from Claude Code credentials, while organization/API usage belongs to Anthropic's Admin Usage and Cost APIs and is intentionally separate from local transcript scanning.
...
Tokens treats `chat` spans as the source of truth for token accounting and ignores tool spans plus cumulative metrics in phase 1:
...
> Copilot's OTEL payloads currently do not expose stable workspace metadata, so Copilot rows may appear without workspace attribution. Tokens prices these rows from the reported model when possible and does not trust `github.copilot.cost` directly.
...
Location: `~/.config/tokens/cursor-cache/usage*.csv` (synced via Cursor API)

Cursor data is fetched from the Cursor API using your session token and cached locally. Tokens reads those cache files for reports; it does not parse local `~/.cursor` session data. See [Cursor IDE Commands](#cursor-ide-commands) for setup.

### Antigravity

Location: `~/.config/tokens/antigravity-cache/sessions/*.jsonl` (synced via local Antigravity language server RPC)

Antigravity data is not fetched automatically by the root command. Run `tokens antigravity sync` while the Antigravity-enabled editor is open to refresh the local cache, then use normal tokens reports and filters against the cached JSONL artifacts.

### Trae

Location: `~/.config/tokens/trae-cache/sessions/*.json` (synced via official usage API)

Trae data is not fetched automatically by the root command. Run `tokens trae login` once, then `tokens trae sync` before reports. Tokens parses the synced API dumps as session-level records and preserves the cost totals reported by Trae.

### Warp/Oz

Location: `~/.config/tokens/warp-cache/usage.json` (synced via authenticated GraphQL API)

Warp/Oz data is not fetched automatically by the root command. Run `tokens warp login`, then `tokens warp sync` before reports. Tokens records only aggregate request counts and spend because Warp does not expose token-attributed local transcripts.

### Grok Build

Location: `$GROK_HOME/sessions/*/*/updates.jsonl` (fallback: `~/.grok/sessions/*/*/updates.jsonl`)

Grok Build data is parsed directly from local session updates. Current logs expose cumulative `totalTokens` counters without a stable input/output split, so Tokens records positive per-turn deltas as input tokens. `grok-composer-2.5-fast` is temporarily mapped to the Composer 2.5 Fast pricing override until a dedicated public price is available.
...
Hermes stores session-level usage in a SQLite `sessions` table. Tokens imports rows where `model` is present and token or cost totals are non-zero, uses `started_at` as the timestamp, preserves `message_count`, and prefers `actual_cost_usd` over `estimated_cost_usd`.
...
`ui_messages.json` is an array of UI events. Tokens counts only:
- `type == "say"`
- `say == "api_req_started"`
...
Kilo uses the same task log shape as Roo Code. Tokens applies the same rules:
...
Cline is the upstream project that Roo Code and Kilo forked from, so it uses the same VS Code globalStorage task log shape. Tokens applies the same rules:
- count only `say/api_req_started` events from `ui_messages.json`
- parse `tokensIn`, `tokensOut`, `cacheReads`, `cacheWrites`, `cost`, and `apiProtocol` from `text` JSON
- enrich model/agent metadata from sibling `api_conversation_history.json` when available
...
- Model names use `provider:model` format (e.g., `anthropic:claude-opus-4-6`) — tokens strips the provider prefix for model identification
...
Crush stores usage in per-project SQLite databases (`crush.db`). Tokens imports session-level cost totals from root sessions only, because Crush does not expose reliable per-message or per-model token accounting. Records appear as `model=session-total` with zero token breakdown.
...
Goose stores per-session usage in a SQLite `sessions.db`. Tokens extracts the model from `model_config_json`, the provider from `provider_name`, and accumulated input/output token totals per session. Reasoning tokens are inferred when the column is populated.
...
JSONL format with a session header and message entries. Tokens emits only assistant messages and reuses gjc's authoritative per-message `usage.cost.total` (USD) when present, recomputing from tokens only when it is absent:
```json
...
Tokens also checks Octofriend SQLite at `~/.local/share/octofriend/sqlite.db` and parses token-bearing records when available.

### MiMo Code

Location: `~/.local/share/mimocode/mimocode.db` (XDG data directory)

MiMo Code stores session data in a SQLite database. Tokens queries the `message` table joined with `session` for workspace context:
...
- LiteLLM cache: `~/.config/tokens/cache/pricing-litellm.json`
- OpenRouter cache: `~/.config/tokens/cache/pricing-openrouter.json` (caches author pricing for models from supported providers)
