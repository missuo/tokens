...
- Primary navigation: Existing Tokens application navigation remains unchanged. `/leaderboard` uses link-based Users/Groups navigation so URLs, history, and server rendering stay authoritative.
...
- Reference, not replica: Adopt the reference's narrow content measure, restrained borders, chart-first composition, and low-noise controls while retaining Tokens typography, data, and blue accent.
...
- Color: Dark zinc-neutral canvas; raised surfaces only slightly lighter; translucent white borders; white/default/muted text with WCAG AA contrast; Tokens blue for the single primary action and selected data emphasis; provider colors only in chart/legend context.
